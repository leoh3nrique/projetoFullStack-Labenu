    ...
    public getPosts = async (input: GetPostInputDto) => {
        ...
        const posts = await Promise.all(
            data.map(async (post) => {
                ...
                const commentsCount = await this.postDatabase.countHowMuchCommentsHave(postWithCreatorName.getId())
                postWithCreatorName.setLikes(Number(likeCount.contagem));
                postWithCreatorName.setDislikes(Number(dislikeCount.contagem));
                postWithCreatorName.setComments(Number(commentsCount.contagem))
